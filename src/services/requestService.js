...
function normalizeMethod(m) {
  const v = String(m || "")
    ...
    .toUpperCase();
  if (!v) return null;
  // Keep it strict so UI/backend match
  const allowed = new Set(["CASH", "MOMO", "BANK", "CARD", "OTHER", "CREDIT"]);
 ...
 *
 * Accepts BOTH {sellerId} or {userId} so controller mismatch won't break.
 * Also stores paymentMethod on sale (requires sales.paymentMethod column).
 */
async function markSale({
  ...
  const actorId = Number(
    sellerId != null ? sellerId : userId != null ? userId : 0,
  );
  ...
  return db.transaction(async (tx) => {
    ...
    // ✅ FIX: compare against the authenticated actor
    if (Number(sale.sellerId) !== actorId) {
      const err = new Error("Forbidden");
      err.code = "FORBIDDEN";
      err.debug = { saleSellerId: Number(sale.sellerId), actorId };
      throw err;
    }
    ...
    const methodNorm = normalizeMethod(paymentMethod);

    // Idempotent no-op if already desired status AND no new method to save
    const alreadyNext = current === nextStatus;
    const alreadyMethod =
      methodNorm == null ||
      String(sale.paymentMethod || sale.payment_method || "").toUpperCase() ===
        methodNorm;

    if (alreadyNext && alreadyMethod) {
      return sale;
    }

    const patch = { updatedAt: new Date(), status: nextStatus };

    // ✅ Only write when provided and valid
    if (methodNorm) {
      // Requires sales.paymentMethod column
      patch.paymentMethod = methodNorm;
    }
 ...
 * - If already fulfilled (or later), restore inventory
